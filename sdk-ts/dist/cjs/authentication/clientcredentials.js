...
class ClientCredentials {
    ...
    async fetchWithTimeoutAndRetry(url, options, timeout, level = 0) {
        if (level > 10) {
        ...
        try {
            ...
            console.warn("Fetch with timeout and retry", url, options, timeout, level, Date.now() - start);
            return response;
        }
        catch (e) {
    ...
    async refreshInternal() {
        ...
        if (this.credentials.client_credentials && time_since_refresh > expires_in / 2) {
